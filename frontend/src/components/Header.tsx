...
import { FaBell, FaShoppingCart, FaChevronDown, FaChevronRight, FaBars } from "react-icons/fa";
...
export const Header = () => {
    ...
    const selectedRoute = route.pathname === "/" ? "Home" : route.pathname.split("/")[1].charAt(0).toUpperCase() + route.pathname.split("/")[1].slice(1);

    const renderDropDown = (navigations: { title: string; link: string }[], level = 0) => {
        return (
            <ul
                ...
                {navigations.map((nav: any, index) => {
                    return nav.children ? (
                        <div className={`dropdown inline-block relative`} key={index}>
                            <MenuItem key={index} className="flex items-center hover:text-[--primary-color] bg-white hover:bg-white py-2 px-4 block text-xs">
                        ...
                        <MenuItem key={index} className="hover:text-[--primary-color] bg-white hover:bg-white py-2 px-4 block text-xs ">
    ...
    const renderNavigation = () => {
        return NAVIGATION.map((nav, index) => (
            <div key={index} className={`hover:text-[--primary-color] ${selectedRoute === nav.title ? "text-[--primary-color]" : ""}`}>
                {nav.children ? (
    ...
    return (
        <div style={{ zIndex: 99 }} className="w-full px-3 bg-white/50 shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px]">
            <div className="h-[52px] w-[94%] mx-auto flex flex-row items-center justify-between ">
                <div className="flex flex-row items-center ">
                    ...
                    <div className="flex flex-row gap-4 justify-evenly text-xs font-semibold max-md:hidden">{renderNavigation()}</div>
                </div>
                ...
                <div className="h-full flex flex-row items-center h-10 ">
                    ...
                    <div className="w-8 h-8 rounded flex items-center justify-center mr-2 mx-3 cursor-pointer hover:bg-[#ebedf0] max-sm:hidden" ref={cartRef}>
                    ...
                    <div className="h-full pl-2 flex flex-row items-center cursor-pointer hover:bg-[#ebedf0]" ref={userRef} onClick={() => setShowUserMenu(true)}>
            ...
            >
                ...
                <MenuItem className="hover:text-[--primary-color] text-sm">Thông tin tài khoản</MenuItem>
                <MenuItem className="hover:text-[--primary-color] text-sm">Đăng xuất</MenuItem>
            </CustomMenu>
            <CustomMenu
            ...
            <Drawer open={showDrawer} onClose={() => setShowDrawer(false)}>
                <Box sx={{ p: 2, width: "250px" }}>
                    ...
                    <div className="flex flex-col text-xs font-semibold gap-4">{renderNavigation()}</div>
                </Box>
                <Divider />
                ...
                <MenuItem className="hover:text-[--primary-color] text-sm">Logout</MenuItem>
            </Drawer>
        </div>
    );
};
